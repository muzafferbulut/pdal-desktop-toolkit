...
class SqlHighlighter(QSyntaxHighlighter):
    ...
    def __init__(self, parent=None):
        ...
        funcs = [
            ...
            r"\bPC_Get\b",
            r"\bPC_Patch\b",
            r"\bST_MakeEnvelope\b",
            r"\bPC_Summary\b",
            r"\bPC_Explode\b",
            r"\bST_Transform\b",
            r"\bPC_FilterEquals\b",
            r"\bPC_FilterBetween\b",
        ]
        for f in funcs:
...
class NewConnectionDialog(QDialog):
    ...
    def _setup_ui(self):
        l = QVBoxLayout(self)
        f = QFormLayout()
        self.le_name, self.le_host, self.le_port = (
            QLineEdit("Local DB"),
            QLineEdit("localhost"),
            QLineEdit("5432"),
        )
        self.le_db, self.le_user, self.le_pass = (
            QLineEdit(),
            QLineEdit("postgres"),
            QLineEdit(),
        )
        self.le_pass.setEchoMode(QLineEdit.Password)
        f.addRow("Name:", self.le_name)
        f.addRow("Host:", self.le_host)
        f.addRow("Port:", self.le_port)
        f.addRow("DB:", self.le_db)
        f.addRow("User:", self.le_user)
        f.addRow("Pass:", self.le_pass)
        l.addLayout(f)
        btn_t = QPushButton("Test Connection")
        btn_t.clicked.connect(self._on_test)
        l.addWidget(btn_t)
...
class DbManagerDialog(QDialog):
    ...
    def __init__(self, data_controller, parent=None):
        ...
        self.active_inspector, self.current_schema, self.current_table = (
            None,
            None,
            None,
        )
    ...
    def _setup_ui(self):
        ...
        tb.addAction(
            QAction(
                QIcon("ui/resources/icons/add_connection.png"),
                "New",
                self,
                triggered=self._open_new_conn_dialog,
            )
        )
        tb.addAction(
            QAction(
                QIcon("ui/resources/icons/refresh.png"),
                "Refresh",
                self,
                triggered=self._load_connections,
            )
        )
        tb.addSeparator()

        self.action_export = QAction(
            QIcon("ui/resources/icons/send_to.png"), " Send to DB", self
        )
        self.action_export.triggered.connect(self._action_export_active_layer)
        tb.addAction(self.action_export)
        self.refresh_layer_name()
        ...
        rl = QVBoxLayout(rp)
        sp.addWidget(rp)
        self.lbl_table = QLabel("SQL Editor")
        rl.addWidget(self.lbl_table)
        ...
        bl = QHBoxLayout()
        rl.addLayout(bl)
        btn_r = QPushButton("Execute SQL")
        btn_r.setIcon(QIcon("ui/resources/icons/run.png"))
        btn_r.clicked.connect(self._run_sql_query)
        bl.addWidget(btn_r)
        ...
        self.btn_load = QPushButton("Load as Layer")
        self.btn_load.setIcon(QIcon("ui/resources/icons/load_to.png"))
        self.btn_load.setEnabled(False)
        self.btn_load.clicked.connect(self._on_load_to_canvas)
    ...
    def refresh_layer_name(self):
        ...
        layer_name = (
            os.path.basename(active_path) if active_path else "No Layer Selected"
        )
    ...
    def _inject_spatial_sql(self, minx, miny, maxx, maxy):
        ...
        srid = self.active_inspector.get_table_srid(
            self.current_schema, self.current_table
        )

        query = (
            f'SELECT * FROM "{self.current_schema}"."{self.current_table}"\n'
            f"WHERE PC_Intersects(patch, \n"
            f"  ST_Transform(ST_MakeEnvelope({minx:.6f}, {miny:.6f}, {maxx:.6f}, {maxy:.6f}, 4326), {srid})\n"
            f") LIMIT 100"
        )
        ...
        main_win = self.parent()
        if hasattr(main_win, "map_view"):
            main_win.map_view.page().runJavaScript("window.clearDrawingsJS();")

    def _on_item_clicked(self, i, c):
        d = i.data(0, Qt.UserRole)
        if d.get("type") in ["table", "view"]:
            self.current_schema, self.current_table, self.active_inspector = (
                d["schema"],
                d["name"],
                DbInspector(d["conn"]),
            )
            self.lbl_table.setText(
                f"Active Object: {self.current_schema}.{self.current_table}"
            )
    ...
    def _load_connections(self):
        ...
        for c in self.repository.get_connections():
            i = QTreeWidgetItem(self.tree, [c["name"]])
            i.setIcon(0, self.style().standardIcon(QStyle.SP_DriveNetIcon))
            i.setData(0, Qt.UserRole, {"type": "connection", "data": c})
            QTreeWidgetItem(i)

    def _open_new_conn_dialog(self):
        dlg = NewConnectionDialog(self)
        if dlg.exec_() and self.repository.save_connection(dlg.conn_data):
            self._load_connections()
    ...
    def _run_sql_query(self):
        ...
        self.w.finished.connect(lambda: self.result_view.setDisabled(False))
        self.w.start()

    def _on_query_success(self, df):
        m = QStandardItemModel(df.shape[0], df.shape[1])
        m.setHorizontalHeaderLabels(list(df.columns))
        for r in range(df.shape[0]):
            for c in range(df.shape[1]):
                m.setItem(r, c, QStandardItem(str(df.iat[r, c])))
        self.result_view.setModel(m)
    ...
    def _on_load_to_canvas(self):
        ...
        self.data_controller.load_from_database(
            self.active_inspector.conn_info, self.current_schema, self.current_table, wh
        )
        self.close()
        ...
        try:
            ...
            if d["type"] == "connection":
                for s in insp.get_schemas():
                    if s.startswith("pg_") or s in ["information_schema"]:
                        continue
                    c = QTreeWidgetItem(i, [s])
                    c.setData(
                        0, Qt.UserRole, {"type": "schema", "name": s, "conn": d["data"]}
                    )
            ...
            QMessageBox.critical(self, "Error", f"Browser error: {str(e)}")
    ...
    def _show_context_menu(self, pos):
        ...
        if d["type"] == "connection":
            ...
            act_del = menu.addAction(
                QIcon("ui/resources/icons/remove.png"), "Delete Connection"
            )
            res = menu.exec_(self.tree.mapToGlobal(pos))
            if res == act_del:
                if self.repository.delete_connection(d["data"]["id"]):
                    self._load_connections()
            elif res == act_new:
                self._create_new_schema(d["data"])
        elif d["type"] == "schema":
            act_table = menu.addAction(
                QIcon("ui/resources/icons/table.png"), "New Table"
            )
            res = menu.exec_(self.tree.mapToGlobal(pos))
            if res == act_table:
    ...
    def _create_new_table(self, conn_info, schema_name):
        name, ok = QInputDialog.getText(
            self, "New Table", f"Create table in {schema_name}:"
        )
        if ok and name.strip():
            ...
            if res["status"]:
                ...
                QMessageBox.critical(self, "Error", res.get("error"))

    def _action_import_file(self):
        if not self.current_table or not self.active_inspector.validate_pc_table(
            self.current_schema, self.current_table
        ):
            return
        ...
        if f:
            self.data_controller.import_layer_to_db(
                f,
                self.active_inspector.conn_info,
                self.current_schema,
                self.current_table,
            )

    def _action_export_active_layer(self):
        if self.current_table and self.active_inspector.validate_pc_table(
            self.current_schema, self.current_table
        ):
            self.data_controller.export_active_layer_to_db(
                self.active_inspector.conn_info, self.current_schema, self.current_table
            )
